...
const difficultyOptions = Object.entries(PlanDifficulty).map((kv) => ({
  value: kv[1],
}));
                ...
                {({ field, form }: FieldProps<FormModel>) => (
                  ...
                  >
                    {difficultyOptions.map((d) => (
                      <Radio key={d.value} value={d.value} color={getDifficultyBadgeColor(d.value)}>
                        {t(`plans.difficulty.${d.value}`)}
                      </Radio>
                    ))}
                  </Radio.Group>
                )}
